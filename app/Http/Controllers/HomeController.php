...
class HomeController extends Controller
{
    public function home()
    {
        ...
        $listSport = $listProducts = Product::where('category_id', 2)
            ->orderBy('id', 'DESC')
            ->limit(4)
            ->get();

        $listPolo = $listProducts = Product::where('category_id', 3)
            ->orderBy('id', 'DESC')
            ->limit(4)
            ->get();;

        return view('fe.home', compact('categories', 'products', 'listNewProduct', 'listCoatProduct', 'listSport', 'listPolo'));
    }
