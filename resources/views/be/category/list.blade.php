...
@section('content')
    <div class="col-lg-12">
        ...
        <div class="text-right" >
            <button class="btn" data-toggle="modal" data-target="#modalinsert"> Thêm</button>
        </div>
        <div><hr></div>
            ...
            <table class="table table-bordered table-hover" id="dataTable">
                ...
                @foreach($list as $item)
                    <tr>
                        ...
                        <td>{{$item->name}}</td>
                        <td>
                            <button array="{{$item}}" id="{{$item->id}}" class="editcategory btn btn-link">Sửa</button>
                            <a class="btn btn-link" href="{{route('admin.category.delete',['id'=>$item->id])}}" onclick="return confirm('Bạn có muốn xoá ?')">Xóa</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
